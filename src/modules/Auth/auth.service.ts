...
class AuthService {
  ...
  protectedRoutes = expressAsyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (user.passwordChangedAt instanceof Date) {
        const changePasswordTime = Math.trunc(
  ...
  verifyResetCode = expressAsyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      let token: string = "";
      if (
      ...
      if (!user)
        return next(new ApiError(`${req.__("check_code_valid")}`, 403)); //
  ...
  resetPassword = expressAsyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!user)
        return next(new ApiError(`${req.__("check_code_verify")}`, 403));
