...
orderSchema.pre("save", async function (next) {
  if (this.isNew) {
    ...
    while (!isUnique) {
      ...
      const existingOrder = await (this.constructor as any).findOne({
        orderCode,
      });
    ...
    if (this.type === OrderTypesEnum.DINEIN && !this?.tableNumber) {
      return next(new ApiError("يجب تحديد الطاولة", 400));
    }
